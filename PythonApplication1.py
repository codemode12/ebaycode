...
#Just to check the URL as it is running 
print(url)
...
while countNum < 5:
    ...
    for i in range (0, len(names)):
        ...
        oldSearchName = (names[i].text).replace(' ', '+')
        ...
        searchName = oldSearchName.replace('Shop+on+eBay', '')
        ...
        #very similar to for loop this is embedded in. iterates through each price
        for j in range (0, soldNameLen):
            ...
            if len(soldPrices[j].text.replace('$', '').replace(',', '')) < 7:
                ...
                if soldPriceLen == 0:
                    ...
                    if origPrice/avgSoldPrice < 0.7 :
                        print(names[i].text + '   Worth Checking At: ' + str(origPrice) + 'VS ' + str(avgSoldPrice))
            else:
                print('Bad Pricer')
